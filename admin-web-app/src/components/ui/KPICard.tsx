...
/**
 * Ported from `admin/components/ui/KPICard.tsx`. Presentational JSX/classNames
 * are unchanged; the admin `useHasMounted` hook (not present in web-app) is
 * inlined below since it was only used to gate the count-up animation.
 */
...
function useHasMounted() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time client-mount flag, mirrors admin/hooks/useHasMounted.ts
    setMounted(true);
  }, []);
  return mounted;
}
...
function CountUp({
  ...
  useEffect(() => {
    if (!isNumeric || reduceMotion) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- syncing to a prop change, not a mount-time set
      setDisplay(value);
      return;
    }
