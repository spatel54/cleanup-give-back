...
/** Ported verbatim from `admin/components/ui/TrendAreaChart.tsx`. */
import { Area, AreaChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
import { motion, useReducedMotion } from 'framer-motion';
import { useHasMounted } from '@/hooks/useHasMounted';
import type { TrendPoint } from '@/lib/mock-data';

const shell = 'bg-bg-surface border border-border-outline rounded-md p-lg flex h-full flex-col gap-md';
...
function TrendTooltip({
  ...
  return (
    <div className="bg-bg-surface border border-border-outline rounded-sm px-md py-sm shadow-bar-top">
      <p className="font-data text-[11px] uppercase tracking-[0.5px] text-text-tertiary mb-xs">{label}</p>
      {payload.map((p) => (
  ...
  const body = (
    <>
      <div>
        ...
        {subtitle && <p className="font-body text-[12px] text-text-tertiary mt-xs">{subtitle}</p>}
      </div>
      {!hasData ? (
        ...
        <div className="flex min-h-0 flex-1 flex-col justify-center gap-md">
          <div
            className="relative h-52 w-full shrink-0"
            role="img"
          ...
          <ul className="flex flex-wrap gap-md shrink-0" aria-hidden>
