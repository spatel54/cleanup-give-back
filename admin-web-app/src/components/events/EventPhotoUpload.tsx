...
/**
 * Ported from `admin/components/events/EventPhotoUpload.tsx`.
 * Multi-select camera/library previews + optional image URL paste. No upload
 * persistence yet - web-app has no backend; files stay client-side for the form UI.
 */
...
export function EventPhotoUpload({ defaultUrls = [] }: { defaultUrls?: string[] }) {
  const inputId = useId();
  ...
  return (
    <div>
        ...
        <ul className="mb-sm grid grid-cols-2 sm:grid-cols-3 gap-sm">
          {keptUrls.map((url, index) => (
            ...
            >
              ...
              <img src={url} alt={`Event photo ${index + 1}`} className="w-full h-full object-cover" />
          ...
          {pending.map((item, index) => (
            ...
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={item.preview} alt={`New photo ${index + 1}`} className="w-full h-full object-cover" />
      ...
      >
        ...
        {totalCount === 0 ? "Add photos (optional)" : atLimit ? "Photo limit reached" : "Add more photos"}
      </label>
      <input
        ref={fileInputRef}
        id={inputId}
