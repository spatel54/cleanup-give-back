...
export function getShipFromAddress(): ShipFromAddress {
  return {
    ...
    street1: process.env.SHIP_FROM_STREET1?.trim() || '600 East Algonquin Road',
    city: process.env.SHIP_FROM_CITY?.trim() || 'Des Plaines',
    state: process.env.SHIP_FROM_STATE?.trim() || 'IL',
    zip: process.env.SHIP_FROM_ZIP?.trim() || '60016',
    country: 'US',
